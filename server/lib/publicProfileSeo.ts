type SalonProfile = {
  ...
  team: Array<{ id: string; name: string; bio: string | null; jobTitle: string | null; specialty: string | null; experienceYears: number | null; avatarUrl: string | null; avatarAltKa: string | null }>;
...
export function renderPublicSalonProfileSnapshot(profile: SalonProfile, origin: string) {
  ...
  const team = profile.team.length ? profile.team.map(member => `<article class="rounded-2xl border bg-card/45 p-4"><div class="flex items-center gap-3">${member.avatarUrl ? `<img src="${escapeHtml(member.avatarUrl)}" alt="${escapeHtml(member.avatarAltKa || `${member.name}-ის ავატარი`)}" class="size-14 rounded-2xl object-cover" />` : ""}<div><h3 class="font-semibold">${escapeHtml(member.name)}</h3><p class="text-sm text-muted-foreground">${escapeHtml(member.jobTitle || member.specialty || "სპეციალისტი")}</p>${member.experienceYears ? `<p class="mt-1 text-xs text-muted-foreground">${member.experienceYears} წლის გამოცდილება</p>` : ""}</div></div>${member.bio ? `<p class="mt-3 text-sm leading-6 text-muted-foreground">${escapeHtml(member.bio)}</p>` : ""}</article>`).join("") : `<p class="text-sm text-muted-foreground">საჯარო გუნდის პროფილები ჯერ არ არის დამატებული.</p>`;
