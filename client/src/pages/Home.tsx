...
export default function Home() {
  return <div className="sf-public-page">
    ...
    <main>
      ...
      <section id="features" className="bg-sidebar py-20 text-sidebar-foreground lg:py-28"><div className="sf-public-container"><div className="max-w-2xl"><p className="sf-kicker text-[var(--sf-terracotta-strong)]">რეალური სამუშაო მოდულები</p><h2 className="sf-display mt-4 text-4xl font-semibold leading-tight sm:text-5xl">პროდუქტი, რომელსაც თქვენი ოპერაციები უკვე იცნობს.</h2><p className="mt-5 text-base leading-7 text-sidebar-foreground/68">ყოველი სივრცე მიბმულია მიმდინარე როლზე, ფილიალსა და იმ მონაცემზე, რომელიც რეალურად არსებობს სისტემაში.</p></div><div className="mt-10 grid gap-4 md:grid-cols-2">{featureStories.map(({ icon: Icon, title, text, label, tone }) => <article key={title} className="rounded-[var(--sf-radius-surface)] border border-sidebar-border/90 bg-sidebar-accent/45 p-6"><span className={`grid size-10 place-items-center rounded-xl ${tone === "jade" ? "bg-[var(--sf-jade-soft)] text-[var(--sf-jade)]" : tone === "terracotta" ? "bg-[color-mix(in_srgb,var(--sf-terracotta)_18%,transparent)] text-[var(--sf-terracotta-strong)]" : tone === "violet" ? "bg-[color-mix(in_srgb,var(--sf-violet)_18%,transparent)] text-[var(--sf-violet)]" : "bg-[color-mix(in_srgb,var(--sf-amber)_18%,transparent)] text-[var(--sf-amber)]"}`}><Icon className="size-5" aria-hidden="true" /></span><p className="mt-6 text-xs font-bold uppercase tracking-[0.11em] text-sidebar-foreground/45">{label}</p><h3 className="mt-2 text-xl font-bold">{title}</h3><p className="mt-3 text-sm leading-6 text-sidebar-foreground/68">{text}</p></article>)}</div></div></section>
