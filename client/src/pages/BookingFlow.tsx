...
export default function BookingFlow() {
  ...
  return <main className="sf-public-page sf-booking-flow px-4 py-6 pb-36 sm:px-6 sm:py-10 lg:pb-10"><div className="mx-auto max-w-6xl">
    ...
    <ol className="sf-booking-progress mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4" aria-label={ui.progress}>{ui.steps.map((label, index) => <li key={label} aria-current={index === step ? "step" : undefined} className={`sf-booking-progress__step flex min-h-12 items-center gap-2 rounded-[var(--sf-radius-control)] border px-3 py-2.5 text-xs sm:min-h-0 sm:py-3 sm:text-sm ${index === step ? "is-current border-[var(--sf-terracotta)] bg-[color-mix(in_srgb,var(--sf-terracotta)_9%,transparent)] font-semibold text-[var(--sf-terracotta-strong)]" : index < step ? "is-complete border-[color-mix(in_srgb,var(--sf-jade)_28%,transparent)] bg-[color-mix(in_srgb,var(--sf-jade)_6%,transparent)] text-[var(--sf-jade)]" : "border-[var(--sf-line)] bg-[var(--sf-surface)] text-[var(--sf-muted)]"}`}><span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-current/25 text-[0.65rem]">{index < step ? <Check className="h-3 w-3" aria-hidden="true" /> : `0${index + 1}`}</span><span className="min-w-0 leading-4">{label}</span></li>)}</ol>
