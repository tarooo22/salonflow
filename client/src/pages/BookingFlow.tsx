...
export default function BookingFlow() {
  ...
  return <main className="sf-public-page sf-booking-flow px-4 py-6 pb-36 sm:px-6 sm:py-10 lg:pb-10"><div className="mx-auto max-w-6xl">
    ...
    {catalog.data?.onlineBookingAvailable === false ? <Card className="mt-8 border-[color-mix(in_srgb,var(--sf-salon-warm)_42%,transparent)]"><CardHeader><CardTitle>{locale === "ka" ? ui.trialPaused : "Online booking is temporarily unavailable"}</CardTitle></CardHeader><CardContent className="space-y-4 text-sm leading-6 text-muted-foreground"><p>{locale === "ka" ? ui.trialPausedLead : "This salon profile remains available, but it is not accepting new online bookings at this time. Please contact the salon for details."}</p><Button asChild variant="publicSecondary"><Link href={`/salon/${slug}`}>{locale === "ka" ? ui.viewSalon : "View salon profile"}</Link></Button></CardContent></Card> : null}
    ...
    {catalog.data && catalog.data.onlineBookingAvailable !== false && !confirmation ? <div className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_21rem]"><section className="space-y-4"><LocationContext location={catalog.data.location} /><div key={step} className="sf-booking-step-pane">{step === 0 ? <ServiceStep catalog={catalog.data.catalog} selectedIds={serviceIds} onSelect={selectService} error={interactionError?.kind === "service" ? interactionError.description : undefined} /> : null}{step === 1 ? <StaffStep team={eligibleTeam} selectedId={staffProfileId} onSelect={selectStaff} error={interactionError?.kind === "staff" ? interactionError.description : undefined} /> : null}{step === 2 ? <TimeStep slug={slug} serviceId={serviceId} serviceIds={serviceIds} multiService={isMultiService} staffProfileId={staffProfileId} workingHours={catalog.data.location.workingHours} timezone={catalog.data.location.timezone} dateTime={dateTime} onChange={selectDateTime} availability={availability} error={interactionError?.kind === "time" ? interactionError.description : undefined} /> : null}{step === 3 ? <ContactStep firstName={firstName} lastName={lastName} phone={phone} email={email} customerNote={customerNote} termsAccepted={termsAccepted} error={interactionError?.kind === "contact" ? interactionError.description : undefined} onFirstName={value => { setFirstName(value); clearIssue("contact"); }} onLastName={setLastName} onPhone={value => { setPhone(value); clearIssue("contact"); }} onEmail={setEmail} onNote={setCustomerNote} onTerms={value => { setTermsAccepted(value); clearIssue("contact"); }} /> : null}</div></section><aside className="lg:sticky lg:top-6 lg:self-start"><BookingSummary step={step} service={selectedServiceSummary} staff={selectedStaff} staffLabel={staffProfileId === ANY_AVAILABLE ? (availability.data?.staffName ? `${availability.data.staffName} (ავტომატურად შეირჩა)` : "ნებისმიერი თავისუფალი სპეციალისტი") : undefined} startsAt={startsAt} available={availability.data?.available === true} onBack={stepBack} onContinue={advanceBooking} onEdit={targetStep => { setInteractionError(null); setStep(targetStep); }} submitting={commitBooking.isPending || commitMultiBooking.isPending} /></aside><BookingMobileCta step={step} label={step < 3 ? ui.continue : (commitBooking.isPending || commitMultiBooking.isPending) ? ui.submitting : ui.submit} detail={selectedServiceSummary ? selectedServiceSummary.nameKa : ui.pending} disabled={commitBooking.isPending || commitMultiBooking.isPending} onContinue={advanceBooking} /></div> : null}
  </div></main>;
}

export function LocationContext({ location }: { location: BookingLocation }) {
  return <Card className="sf-salon-panel"><CardHeader className="pb-3"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--sf-salon-warm)]">ფილიალი</p><CardTitle className="mt-2">{location.name}</CardTitle>{location.publicDescription ? <p className="mt-2 text-sm leading-6 text-muted-foreground">{location.publicDescription}</p> : null}</CardHeader><CardContent className="space-y-3 text-sm text-muted-foreground"><p className="flex items-center gap-2"><MapPin className="h-4 w-4 shrink-0 text-[var(--sf-salon-warm)]" aria-hidden="true" />{location.address || location.timezone}</p>{location.workingHours.length ? <div className="rounded-xl bg-[var(--sf-surface-hover)] p-3"><p className="flex items-center gap-2 font-medium text-[var(--sf-ink)]"><CalendarClock className="h-4 w-4 text-[var(--sf-salon-warm)]" aria-hidden="true" />ხელმისაწვდომი სპეციალისტების საათები</p><div className="mt-2 grid gap-1 text-xs">{location.workingHours.map(rule => <p key={rule.weekday}>{weekdayName(rule.weekday)} · {rule.startLocalTime}–{rule.endLocalTime}</p>)}</div></div> : <p className="rounded-xl border border-dashed p-3 text-xs leading-5">სამუშაო საათები ჯერ არ არის მითითებული. დროის არჩევისას ხელმისაწვდომობა მაინც ავტომატურად შემოწმდება.</p>}{location.phone ? <a href={`tel:${location.phone}`} className="flex items-center gap-2 font-medium text-[var(--sf-ink)] hover:text-[var(--sf-salon-warm)]"><Phone className="h-4 w-4 text-[var(--sf-salon-warm)]" aria-hidden="true" />{location.phone}</a> : null}{location.email ? <a href={`mailto:${location.email}`} className="flex items-center gap-2 font-medium text-[var(--sf-ink)] hover:text-[var(--sf-salon-warm)]"><Mail className="h-4 w-4 text-[var(--sf-salon-warm)]" aria-hidden="true" />{location.email}</a> : null}</CardContent></Card>;
}
...
function ServiceStep({ catalog, selectedIds, onSelect, error }: { catalog: Array<{ service: BookingService; category: { nameKa: string } }>; selectedIds: string[]; onSelect: (id: string) => void; error?: string }) {
  const categories = useMemo(() => Array.from(new Map(catalog.map(item => [item.category.nameKa, item.category])).values()), [catalog]);
  const [activeCategory, setActiveCategory] = useState("ყველა");
  const visibleCatalog = activeCategory === "ყველა" ? catalog : catalog.filter(item => item.category.nameKa === activeCategory);
  return <Card className="border-[var(--sf-ink)]/10"><BookingStepHeader step={1} title="რა გსურთ დღეს?" description="აირჩიეთ ერთი ან რამდენიმე სერვისი. დრო და ფასი შეჯამდება, შემდეგ კი გამოჩნდება მხოლოდ ყველა არჩეული სერვისისთვის შესაბამისი სპეციალისტი."><span className="rounded-full bg-[var(--sf-accent-strong)]/10 px-3 py-1.5 text-xs font-semibold text-[var(--sf-accent-strong)]" aria-live="polite">{selectedIds.length ? `${selectedIds.length} არჩეული` : "აირჩიეთ სერვისი"}</span></BookingStepHeader><CardContent className="space-y-4">{error ? <InlineIssue>{error}</InlineIssue> : null}{categories.length > 1 ? <div className="flex gap-2 overflow-x-auto pb-1" aria-label="სერვისის კატეგორია"><Button type="button" size="sm" variant={activeCategory === "ყველა" ? "default" : "outline"} className="min-h-11 shrink-0" aria-pressed={activeCategory === "ყველა"} onClick={() => setActiveCategory("ყველა")}>ყველა</Button>{categories.map(category => <Button key={category.nameKa} type="button" size="sm" variant={activeCategory === category.nameKa ? "default" : "outline"} className="min-h-11 shrink-0" aria-pressed={activeCategory === category.nameKa} onClick={() => setActiveCategory(category.nameKa)}>{category.nameKa}</Button>)}</div> : null}{visibleCatalog.length ? <div className="grid gap-3 sm:grid-cols-2">{visibleCatalog.map(({ service, category }) => { const selected = selectedIds.includes(service.id); return <BookingChoiceCard key={service.id} conversionEvent="BOOKING_SERVICE_SELECTED" label={`${service.nameKa} არჩევა`} selected={selected} onSelect={() => onSelect(service.id)}><div className="pr-8"><p className="text-xs font-semibold text-[var(--sf-accent-strong)]">{category.nameKa}</p><p className="mt-1 text-base font-semibold">{service.nameKa}</p></div><div className="mt-4 flex flex-wrap gap-2 text-xs font-medium text-[var(--sf-muted)]"><span className="rounded-full bg-[var(--sf-surface-subtle)] px-2.5 py-1"><Clock3 className="mr-1 inline h-3.5 w-3.5 text-[var(--sf-accent-strong)]" aria-hidden="true" />{service.defaultDurationMinutes} წუთი</span><span className="rounded-full bg-[var(--sf-surface-subtle)] px-2.5 py-1">{formatGel(service.priceTetri)}</span></div></BookingChoiceCard>; })}</div> : <p className="rounded-xl border border-dashed border-[var(--sf-ink)]/15 bg-white/60 p-4 text-sm leading-6 text-muted-foreground">ამ კატეგორიაში ონლაინ სერვისი ჯერ არ არის. აირჩიეთ სხვა კატეგორია ან დაუკავშირდით სალონს.</p>}</CardContent></Card>;
}
...
export function StaffStep({ team, selectedId, onSelect, error }: { team: BookingTeamMember[]; selectedId?: string; onSelect: (id: string) => void; error?: string }) {
  return <Card className="border-[var(--sf-ink)]/10"><BookingStepHeader step={2} title="ვინ მოგემსახუროთ?" description="ჯერ შემოგთავაზებთ სწრაფ გზას — თავისუფალი, შესაბამისი სპეციალისტის ავტომატურ არჩევას. კონკრეტული არჩევანი შეგიძლიათ ქვემოთ გააკეთოთ." /><CardContent className="space-y-3">{error ? <InlineIssue>{error}</InlineIssue> : null}{team.length ? <><BookingChoiceCard conversionEvent="BOOKING_ANY_AVAILABLE_SELECTED" label="ნებისმიერი თავისუფალი სპეციალისტის არჩევა" tone="availability" selected={selectedId === ANY_AVAILABLE} onSelect={() => onSelect(ANY_AVAILABLE)}><div className="pr-8"><p className="font-semibold">ნებისმიერი თავისუფალი სპეციალისტი</p><p className="mt-1 text-sm leading-6 text-[var(--sf-muted)]">სისტემა დროის არჩევისას მოძებნის მხოლოდ არჩეული სერვისებისთვის რეალურად თავისუფალ სპეციალისტს.</p></div><p className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-[var(--sf-jade)]"><CheckCircle2 className="size-3.5" aria-hidden="true" />სწრაფი არჩევანი</p></BookingChoiceCard><div className="pt-2"><p className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--sf-muted)]">ან აირჩიეთ კონკრეტული სპეციალისტი</p><div className="grid gap-3 sm:grid-cols-2">{team.map(member => <BookingChoiceCard key={member.id} conversionEvent="BOOKING_SPECIALIST_SELECTED" label={`${member.name} არჩევა`} selected={member.id === selectedId} onSelect={() => onSelect(member.id)}><div className="pr-8"><p className="font-semibold">{member.name}</p>{member.specialty ? <p className="mt-1 text-sm text-[var(--sf-muted)]">{member.specialty}</p> : null}</div><p className="mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-[var(--sf-jade)]"><CheckCircle2 className="size-3.5" aria-hidden="true" />შესაბამისია არჩეული სერვისებისთვის</p>{member.bio ? <p className="mt-2 text-xs leading-5 text-[var(--sf-muted)]">{member.bio}</p> : null}</BookingChoiceCard>)}</div></div></> : <p className="rounded-xl border border-dashed border-[var(--sf-ink)]/15 bg-white/60 p-4 text-sm leading-6 text-[var(--sf-muted)]">ამ სერვისისთვის ამ ფილიალში აქტიური ონლაინ სპეციალისტი ჯერ არ არის. გთხოვთ აირჩიოთ სხვა სერვისი ან დაუკავშირდეთ სალონს.</p>}</CardContent></Card>;
}
...
function TimeStep({ slug, serviceId, serviceIds, multiService, staffProfileId, workingHours, timezone, dateTime, onChange, availability, error }: { slug: string; serviceId?: string; serviceIds: string[]; multiService: boolean; staffProfileId?: string; workingHours: Array<{ weekday: number }>; timezone: string; dateTime: string; onChange: (value: string) => void; availability: { isLoading: boolean; isError: boolean; data?: { available: boolean; reason?: string } }; error?: string }) {
  ...
  const slotsByPeriod = useMemo(() => ({
    დილა: slots.filter(slot => Number(slot.label.split(":")[0]) < 12),
    შუადღე: slots.filter(slot => { const hour = Number(slot.label.split(":")[0]); return hour >= 12 && hour < 17; }),
    საღამო: slots.filter(slot => Number(slot.label.split(":")[0]) >= 17),
  }), [slots]);

  const pickDate = (day: string) => { setSelectedDate(day); if (dateTime) onChange(""); };

  return <Card className="border-[var(--sf-ink)]/10"><BookingStepHeader step={3} title="რომელი დრო გერგებათ?" description="ნაჩვენებია მხოლოდ რეალურად თავისუფალი საათები. დროის არჩევის შემდეგ ხელმისაწვდომობა კვლავ გადამოწმდება." /><CardContent className="space-y-5">{error ? <InlineIssue>{error}</InlineIssue> : null}
    ...
    <div aria-live="polite" className="min-h-24">
      {!dayIsOpen ? <p className="rounded-xl border border-dashed border-[var(--sf-line)] bg-[var(--sf-surface-subtle)] p-4 text-sm leading-6 text-muted-foreground">ამ დღეს სალონი დახურულია. აირჩიეთ სხვა თარიღი.</p>
        ...
        : slots.length ? <div className="space-y-5"><BookingSlotGroup conversionEvent="BOOKING_TIME_SELECTED" title="დილა" slots={slotsByPeriod.დილა} selectedStartsAt={dateTime} onSelect={onChange} /><BookingSlotGroup conversionEvent="BOOKING_TIME_SELECTED" title="შუადღე" slots={slotsByPeriod.შუადღე} selectedStartsAt={dateTime} onSelect={onChange} /><BookingSlotGroup conversionEvent="BOOKING_TIME_SELECTED" title="საღამო" slots={slotsByPeriod.საღამო} selectedStartsAt={dateTime} onSelect={onChange} /></div>
        : <div className="rounded-xl border border-dashed border-[var(--sf-line)] bg-[var(--sf-surface-subtle)] p-4 text-sm leading-6 text-muted-foreground"><p className="font-medium text-[var(--sf-ink)]">ამ დღისთვის თავისუფალი დრო აღარ არის.</p><p className="mt-1">შეგიძლიათ აირჩიოთ სხვა დღე ან დატოვოთ waitlist მოთხოვნა. ეს არ არის დადასტურებული ჯავშანი და ავტომატური შეტყობინება ჯერ არ იგზავნება.</p><Link href={`/waitlist/${slug}?serviceId=${encodeURIComponent(serviceId ?? "")}&staffProfileId=${encodeURIComponent(staffProfileId ?? "")}&date=${encodeURIComponent(selectedDate)}`} data-conversion-event="BOOKING_WAITLIST_HANDOFF" className="mt-3 inline-flex min-h-11 items-center font-semibold text-[var(--sf-terracotta-strong)] underline-offset-4 hover:underline">Waitlist მოთხოვნა</Link></div>}
    </div>
...
function ContactStep({ firstName, lastName, phone, email, customerNote, termsAccepted, error, onFirstName, onLastName, onPhone, onEmail, onNote, onTerms }: { firstName: string; lastName: string; phone: string; email: string; customerNote: string; termsAccepted: boolean; error?: string; onFirstName: (value: string) => void; onLastName: (value: string) => void; onPhone: (value: string) => void; onEmail: (value: string) => void; onNote: (value: string) => void; onTerms: (value: boolean) => void }) {
  return <Card className="border-[var(--sf-ink)]/10"><BookingStepHeader step={4} title="სად დაგიკავშირდეთ?" description="ვითხოვთ მხოლოდ იმ მონაცემებს, რომლებიც საჭიროა ამ მოთხოვნის დამუშავებისა და სალონის დადასტურებისთვის." /><CardContent className="space-y-4">{error ? <InlineIssue>{error}</InlineIssue> : null}<div className="grid gap-3 sm:grid-cols-2"><label className="space-y-2 text-sm font-medium" htmlFor="booking-first-name"><span>სახელი *</span><Input id="booking-first-name" value={firstName} onChange={event => onFirstName(event.target.value)} autoComplete="given-name" required aria-invalid={Boolean(error && !firstName.trim())} /></label><label className="space-y-2 text-sm font-medium" htmlFor="booking-last-name"><span>გვარი <span className="font-normal text-muted-foreground">(არასავალდებულო)</span></span><Input id="booking-last-name" value={lastName} onChange={event => onLastName(event.target.value)} autoComplete="family-name" /></label></div><label className="block space-y-2 text-sm font-medium" htmlFor="booking-phone"><span>მობილურის ნომერი *</span><Input id="booking-phone" type="tel" value={phone} onChange={event => onPhone(event.target.value)} autoComplete="tel" inputMode="tel" placeholder="მაგ. 555 12 34 56" aria-describedby="booking-phone-help" required aria-invalid={Boolean(error && !phone.trim())} /><span id="booking-phone-help" className="block text-xs font-normal leading-5 text-muted-foreground">ფორმატი: 5XX XX XX XX ან +995 5XX XX XX XX. ნომერი გამოყენებული იქნება მხოლოდ ამ მოთხოვნისთვის.</span></label><label className="block space-y-2 text-sm font-medium" htmlFor="booking-email"><span>ელფოსტა <span className="font-normal text-muted-foreground">(არასავალდებულო)</span></span><Input id="booking-email" type="email" value={email} onChange={event => onEmail(event.target.value)} autoComplete="email" /></label><label className="block space-y-2 text-sm font-medium" htmlFor="booking-note"><span>კომენტარი სალონისთვის <span className="font-normal text-muted-foreground">(არასავალდებულო)</span></span><Textarea id="booking-note" value={customerNote} onChange={event => onNote(event.target.value)} /></label><label className={`flex items-start gap-3 rounded-xl p-3 text-sm leading-5 text-[var(--sf-muted)] ${error && !termsAccepted ? "border border-destructive/35 bg-destructive/5" : "bg-[var(--sf-surface-hover)]"}`}><Checkbox checked={termsAccepted} onCheckedChange={value => onTerms(value === true)} aria-invalid={Boolean(error && !termsAccepted)} /><span>ვეთანხმები, რომ სალონმა ჩემი მონაცემები გამოიყენოს მხოლოდ ამ ჯავშნის მოთხოვნის დამუშავებისა და დადასტურებისთვის. *</span></label></CardContent></Card>;
}

function BookingSummary({ step, service, staff, staffLabel, startsAt, available, onBack, onContinue, onEdit, submitting }: { step: number; service?: BookingService; staff?: BookingTeamMember; staffLabel?: string; startsAt: Date | null; available: boolean; onBack: () => void; onContinue: () => void; onEdit: (step: number) => void; submitting: boolean }) {
  const { locale } = usePublicLocale();
  const ui = bookingUi[locale];
  const rows = [{ label: ui.service, value: service ? `${service.nameKa} · ${formatGel(service.priceTetri)}` : ui.pending, ready: Boolean(service), editStep: 0 }, { label: ui.specialist, value: staffLabel ?? staff?.name ?? ui.selectSpecialist, ready: Boolean(staff || staffLabel), editStep: 1 }, { label: ui.time, value: startsAt ? formatKaDateTime(startsAt) : ui.selectTime, ready: Boolean(startsAt && available), editStep: 2 }];
  return <div className="space-y-4"><BookingVisitSummary rows={rows} onEdit={onEdit} title={ui.choice} description={ui.summary} /><Card className="border-[var(--sf-ink)]/10 bg-white shadow-sm"><CardContent className="space-y-3 p-4">{step > 0 ? <Button variant="outline" className="min-h-11 w-full border-[var(--sf-ink)]/15" onClick={onBack}>{ui.previous}</Button> : null}<Button data-conversion-event={step === 3 ? "BOOKING_SUBMIT_SELECTED" : undefined} className="min-h-11 w-full bg-[var(--sf-accent-strong)] shadow-sm hover:bg-[var(--sf-accent)]" disabled={submitting} onClick={onContinue}>{step < 3 ? ui.continue : submitting ? ui.submitting : ui.submit}</Button></CardContent></Card><Card className="border-[var(--sf-jade)]/20 bg-[var(--sf-jade)]/5"><CardContent className="flex gap-3 p-4 text-sm leading-5 text-[var(--sf-jade)]"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[var(--sf-jade)]" aria-hidden="true" /><p><span className="font-semibold">{ui.protected}</span> {ui.protectedLead}</p></CardContent></Card></div>;
}
...
export function BookingConfirmation({ confirmationToken, assignedStaffName, serviceName, startsAt, endsAt, locationName, locationAddress }: { confirmationToken: string; assignedStaffName?: string; serviceName: string; startsAt: Date; endsAt: Date; locationName: string; locationAddress?: string | null }) {
  ...
  return <Card role="status" aria-live="polite" className="sf-booking-confirmation sf-grain relative mt-8 overflow-hidden border-transparent text-center shadow-[var(--sf-glow-brand)]" style={{ background: "var(--sf-gradient-brand)" }}>
    ...
    <CardContent className="relative flex flex-col items-center gap-4 px-6 py-10 text-white">
      <span className="sf-booking-confirmation__halo grid size-20 place-items-center rounded-full bg-white/20 backdrop-blur-sm"><span className="grid size-14 place-items-center rounded-full bg-white text-[var(--sf-accent-strong)]"><CheckCircle2 className="h-8 w-8" aria-hidden="true" /></span></span>
