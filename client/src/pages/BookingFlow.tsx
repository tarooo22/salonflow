...
export default function BookingFlow() {
  ...
  return <main className="sf-public-page sf-booking-flow px-4 py-6 pb-28 sm:px-6 sm:py-10 lg:pb-10"><div className="mx-auto max-w-6xl">
    ...
    <header className="sf-grain relative mt-5 overflow-hidden rounded-[var(--sf-radius-hero)] px-6 py-7 text-white shadow-[var(--sf-glow-brand)] sm:px-8 sm:py-9" style={{ background: "var(--sf-gradient-brand)" }}><span className="sf-blob" style={{ width: "20rem", height: "20rem", top: "-7rem", right: "-3rem", background: "rgba(255,255,255,0.3)", opacity: 0.55, animation: "sf-blob-morph 16s ease-in-out infinite" }} aria-hidden="true" /><div className="relative"><div className="flex flex-wrap items-center justify-between gap-3"><p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-white/85"><Sparkles className="h-3.5 w-3.5" aria-hidden="true" /> ონლაინ ჩაწერა · 4 ნაბიჯი</p><span className="rounded-full border border-white/30 bg-white/15 px-3 py-1.5 text-xs font-semibold text-white/90">დაცული მოთხოვნა</span></div><h1 className="sf-display mt-5 max-w-2xl text-4xl font-semibold leading-tight sm:text-5xl">დაჯავშნეთ თქვენი მშვიდი დრო.</h1>{catalog.data ? <p className="mt-3 text-sm text-white/80">{catalog.data.location.name} · {catalog.data.location.timezone}</p> : <p className="mt-3 text-sm text-white/80">აირჩიეთ სერვისი და დაგეგმეთ ვიზიტი რამდენიმე მარტივ ნაბიჯში.</p>}</div></header>
...
function BookingSummary({ step, service, staff, staffLabel, startsAt, available, onBack, onContinue, submitting }: { step: number; service?: BookingService; staff?: BookingTeamMember; staffLabel?: string; startsAt: Date | null; available: boolean; onBack: () => void; onContinue: () => void; submitting: boolean }) {
  ...
  return <div className="space-y-4"><Card className="border-[var(--sf-ink)]/10 bg-white shadow-sm"><CardHeader><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[var(--sf-accent-strong)]/10 text-[var(--sf-accent-strong)]"><UserRound className="h-5 w-5" aria-hidden="true" /></span><div><CardTitle>თქვენი არჩევანი</CardTitle><p className="mt-1 text-xs text-muted-foreground">შეჯამება განახლდება თითოეულ ნაბიჯზე.</p></div></div></CardHeader><CardContent className="space-y-3">{rows.map(row => <div key={row.label} className="rounded-xl border border-[var(--sf-ink)]/8 bg-[var(--sf-surface-hover)]/65 p-3"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[var(--sf-muted)]">{row.label}</p><p className={`mt-1 text-sm leading-5 ${row.ready ? "font-medium text-[var(--sf-ink)]" : "text-[var(--sf-muted)]"}`}>{row.value}</p></div>)}{step > 0 ? <Button variant="outline" className="w-full border-[var(--sf-ink)]/15" onClick={onBack}>წინა ნაბიჯი</Button> : null}<Button className="w-full bg-[var(--sf-accent-strong)] shadow-sm hover:bg-[var(--sf-accent)]" disabled={submitting} onClick={onContinue}>{step < 3 ? "გაგრძელება" : submitting ? "იგზავნება…" : "ჯავშნის გაგზავნა"}</Button></CardContent></Card><Card className="border-[var(--sf-jade)]/20 bg-[var(--sf-jade)]/5"><CardContent className="flex gap-3 p-4 text-sm leading-5 text-[var(--sf-jade)]"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[var(--sf-jade)]" aria-hidden="true" /><p><span className="font-semibold">დაცული ონლაინ მოთხოვნა.</span> არჩევანი გადამოწმდება ხელმისაწვდომობასთან, ხოლო საბოლოო დადასტურებას მიიღებთ სალონისგან.</p></CardContent></Card></div>;
}
