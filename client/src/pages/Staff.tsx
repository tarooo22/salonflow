...
import { BriefcaseBusiness, CalendarOff, Check, Clock3, Copy, Link2, Mail, MapPin, Pencil, Plus, Send, Trash2, UsersRound, X } from "lucide-react";
...
export default function Staff() {
  ...
  const invites = trpc.invitations.list.useQuery({ organizationId: organization?.id ?? "" }, { enabled: Boolean(organization?.id && canManage) });
  ...
  const createMember = trpc.staff.createMember.useMutation({
    onSuccess: async () => {
      ...
      setMemberOpen(false); setMemberFullName(""); setMemberDisplayName(""); setMemberJobTitle(""); setMemberRole("STAFF"); setMemberColor("#7C3AED"); setMemberLocationIds([]); setMemberEmail(""); setMemberPassword(""); setMemberError("");
      toast.success("გუნდის ახალი წევრი დაემატა.");
    },
  ...
  const openMemberDialog = () => { setMemberFullName(""); setMemberDisplayName(""); setMemberJobTitle(""); setMemberRole("STAFF"); setMemberColor("#7C3AED"); setMemberLocationIds(locations.data?.length === 1 ? [locations.data[0]!.id] : []); setMemberEmail(""); setMemberPassword(""); setMemberError(""); setMemberOpen(true); };
  const submitMember = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (memberEmail && memberPassword.length < 8) { setMemberError("პაროლი უნდა შეიცავდეს მინიმუმ 8 სიმბოლოს."); return; }
    setMemberError("");
    createMember.mutate({
      organizationId: organization.id,
...
function MemberDialog({ open, setOpen, submit, fullName, setFullName, displayName, setDisplayName, jobTitle, setJobTitle, role, setRole, color, setColor, locations, selected, setSelected, email, setEmail, password, setPassword, error, pending }: any) {
  const toggle = (id: string) => setSelected((current: string[]) => current.includes(id) ? current.filter(value => value !== id) : [...current, id]);
  return <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-lg"><DialogHeader><DialogTitle>ახალი გუნდის წევრი</DialogTitle><DialogDescription>დაამატეთ სპეციალისტი, ადმინისტრატორი ან მენეჯერი. ელფოსტა და პაროლი არასავალდებულოა — თუ მითითებულია, წევრს შეეძლება სისტემაში შესვლა.</DialogDescription></DialogHeader><form onSubmit={submit} className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><Field label="სრული სახელი *"><Input value={fullName} onChange={event => { setFullName(event.target.value); if (!displayName) setDisplayName(event.target.value); }} placeholder="მაგ. ნინო ბერიძე" required minLength={2} /></Field><Field label="საჯარო სახელი *"><Input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="ჩაწერაში გამოსაჩენი სახელი" required minLength={2} /></Field></div><div className="grid gap-3 sm:grid-cols-2"><Field label="როლი"><Select value={role} onValueChange={value => setRole(value as any)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="STAFF">სპეციალისტი</SelectItem><SelectItem value="RECEPTIONIST">ადმინისტრატორი</SelectItem><SelectItem value="MANAGER">მენეჯერი</SelectItem></SelectContent></Select></Field><Field label="სპეციალიზაცია"><Input value={jobTitle} onChange={event => setJobTitle(event.target.value)} placeholder="მაგ. სტილისტი / კოლორისტი" /></Field></div><Field label="ფერი კალენდარში"><div className="flex flex-wrap gap-2">{memberColorPresets.map(preset => <button key={preset} type="button" aria-label={`ფერი ${preset}`} aria-pressed={color === preset} onClick={() => setColor(preset)} className={`size-8 rounded-full transition ${color === preset ? "ring-2 ring-offset-2 ring-primary" : "hover:scale-110"}`} style={{ background: preset }} />)}</div></Field><fieldset className="space-y-2"><legend className="text-sm font-medium">ფილიალები *</legend>{locations.length ? locations.map((location: any) => <label key={location.id} className="flex items-center gap-2 rounded-lg border p-2 hover:bg-muted/40 cursor-pointer"><input type="checkbox" checked={selected.includes(location.id)} onChange={() => toggle(location.id)} className="accent-primary" />{location.name}</label>) : <p className="rounded-lg border border-dashed p-3 text-sm text-muted-foreground">ჯერ არ არის აქტიური ფილიალი.</p>}</fieldset><div className="rounded-xl border border-dashed border-primary/30 bg-primary/5 p-3 space-y-3"><p className="text-xs font-semibold uppercase tracking-wide text-primary">შესვლის მონაცემები (არასავალდებულო)</p><div className="grid gap-3 sm:grid-cols-2"><Field label="ელფოსტა"><Input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="staff@example.com" autoComplete="off" /></Field><Field label="დროებითი პაროლი"><Input type="text" value={password} onChange={event => setPassword(event.target.value)} placeholder="მინიმუმ 8 სიმბოლო" autoComplete="new-password" minLength={email ? 8 : undefined} /></Field></div></div>{error ? <p className="text-sm text-destructive">{error}</p> : null}<DialogFooter><Button type="submit" disabled={pending}>{pending ? "ინახება…" : "წევრის დამატება"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
