      ...
      <div className="mx-auto w-full max-w-7xl space-y-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {organization && canManage && !hasOwnProfile ? <Button onClick={() => { setFormError(""); setCreateOpen(true); }}><Plus className="mr-2 h-4 w-4" />ჩემი პროფილის დამატება</Button> : null}
          </div>
        </header>
        ...
        {organization ? <>
          ...
          <Card><CardHeader><CardTitle>აქტიური გუნდი</CardTitle></CardHeader><CardContent><div className="grid gap-4 lg:grid-cols-2">
            ...
            {!staff.isLoading && !staff.isError && staff.data?.length === 0 ? <div className="rounded-xl border border-dashed p-6 text-sm leading-6 text-muted-foreground"><p>აქ გამოჩნდება გუნდის პროფილები, როგორც კი ფილიალში პირველი სპეციალისტი დაემატება.</p>{canManage ? <Button variant="outline" size="sm" className="mt-4" onClick={() => setCreateOpen(true)}><Plus className="mr-1.5 h-4 w-4" />პირველი პროფილის დამატება</Button> : null}</div> : null}
