      ...
      <div className="mx-auto w-full max-w-4xl space-y-6 py-2 sm:py-8">
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          {step === 2 ? <Card>
            ...
            <CardContent className="space-y-3">{WEEKDAYS.map(day => { const hour = openingHours.find(item => item.weekday === day.weekday)!; return <div key={day.weekday} className="grid gap-3 rounded-xl border border-border p-3 sm:grid-cols-[minmax(9rem,1fr)_auto_auto] sm:items-center"><label className="flex min-h-11 min-w-0 items-center gap-3 text-sm font-medium"><input className="size-4 shrink-0" type="checkbox" checked={hour.enabled} onChange={event => updateHour(day.weekday, { enabled: event.target.checked })} /><span className="truncate">{day.label}</span></label><div className="grid min-w-0 grid-cols-2 gap-2 sm:contents"><Input aria-label={`${day.label} დაწყება`} type="time" value={hour.startLocalTime} onChange={event => updateHour(day.weekday, { startLocalTime: event.target.value })} disabled={!hour.enabled} className="h-11 w-full min-w-0" /><Input aria-label={`${day.label} დასრულება`} type="time" value={hour.endLocalTime} onChange={event => updateHour(day.weekday, { endLocalTime: event.target.value })} disabled={!hour.enabled} className="h-11 w-full min-w-0" /></div></div>; })}<p className="rounded-xl bg-muted p-3 text-xs leading-5 text-muted-foreground">პირველი გამონაკლისი (შვებულება, პაუზა ან დახურვა) სურვილისამებრ შეგიძლიათ დაამატოთ გუნდის გვერდიდან სამუშაო სივრცის შექმნის შემდეგ.</p></CardContent>
          </Card> : null}
