...
export default function MarketplaceSetup() {
  ...
  return <DashboardLayout><main className="sf-workspace-page mx-auto w-full max-w-7xl space-y-5">
    ...
    {isOwner ? <>
      ...
      {locationId ? <>
        {listing.data?.launchReadiness ? <WorkspaceSection title="საჯარო პროფილის მზადყოფნა" description="ეს checklist არ აქვეყნებს სალონს ავტომატურად. იგი მხოლოდ გაჩვენებთ, რა უნდა შეავსოთ, სანამ რეალურ listing-ს review-ზე გაგზავნით."><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">{listing.data.launchReadiness.completed}/{listing.data.launchReadiness.total} პუნქტი მზადაა</p><p className="mt-1 text-xs text-muted-foreground">{listing.data.launchReadiness.readyForReview ? "საბაზისო ინფორმაცია სრულადაა მითითებული; შემდეგი ნაბიჯია review-ზე გაგზავნა." : "დაასრულეთ მონიშნული პუნქტები, რათა public profile სანდოდ გამოჩნდეს."}</p></div><WorkspaceStatusPill tone={listing.data.launchReadiness.readyForReview ? "success" : "warning"}>{listing.data.launchReadiness.readyForReview ? "მზადაა review-სთვის" : "საჭიროა შევსება"}</WorkspaceStatusPill></div><div className="mt-4 grid gap-2 md:grid-cols-2">{listing.data.launchReadiness.items.map(item => <article key={item.key} className={`rounded-xl border p-3 ${item.complete ? "border-emerald-500/25 bg-emerald-500/5" : "bg-muted/10"}`}><div className="flex items-center gap-2 text-sm font-semibold">{item.complete ? <CircleCheck className="size-4 text-emerald-700" /> : <span className="grid size-4 place-items-center rounded-full border text-[10px] text-muted-foreground" aria-hidden="true">!</span>}{item.label}</div>{!item.complete ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{item.action}</p> : <p className="mt-1 text-xs text-muted-foreground">მითითებულია</p>}</article>)}</div></WorkspaceSection> : null}
