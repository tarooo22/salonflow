...
export function guidedTourSteps(role: TourRole): TourStep[] {
  if (role === "OWNER") return [
    ...
    { title: "ყოველდღიური ოპერაციები", description: "აქ არის attendance, tips და სხვა ყოველდღიური პროცესები. თითოეული მოქმედება რჩება თქვენი როლისა და ფილიალის scope-ში.", path: "/app/operations", actionLabel: "ოპერაციების გახსნა" },
    { title: "POS და მარაგი", description: "პროდუქტის გაყიდვა და მარაგის მოძრაობა გამოიყენეთ მხოლოდ რეალური ოპერაციის დროს; გარე გადახდის თანხის ჩამოჭრა აქ არ აქტიურდება.", path: "/app/pos", actionLabel: "POS-ის გახსნა" },
    { title: "ანგარიშები", description: "ანგარიშებში ნახავთ მხოლოდ ორგანიზაციის რეალურ მონაცემებზე დაფუძნებულ მეტრიკებს. პროგნოზი booking-ების ღირებულების შეფასებაა და არა გარანტირებული ჩარიცხვა.", path: "/app/reports", actionLabel: "ანგარიშების გახსნა" },
    { title: "საჯარო პროფილის მზადყოფნა", description: "მედია და პროფილის გვერდზე შეავსეთ მხოლოდ დამტკიცებული ფოტო, გუნდის ინფორმაცია და კლიენტის ცალკე თანხმობით გამოქვეყნებული შედეგი.", path: "/app/media", actionLabel: "მედიის გახსნა" },
  ...
  if (role === "MANAGER") return [
    ...
    { title: "ოპერაციები", description: "attendance და daily operational actions აქ არის თავმოყრილი. მხოლოდ თქვენი უფლებებით დაშვებული მოქმედებები გამოჩნდება.", path: "/app/operations", actionLabel: "ოპერაციების გახსნა" },
  ];
  if (role === "RECEPTIONIST") return [
    ...
    { title: "ოპერაციები", description: "თუ ადგილზე მოსული კლიენტი ან დღის ოპერაცია გაქვთ, ეს სივრცე გაძლევთ სწრაფ და როლით შეზღუდულ მოქმედებებს.", path: "/app/operations", actionLabel: "ოპერაციების გახსნა" },
  ];
  return [
    ...
    { title: "ჩემი ოპერაციები", description: "attendance და პირადი სამუშაო მოქმედებები ამ გვერდზეა. სხვისი კლიენტები ან ფილიალის მართვა აქ არ ჩანს.", path: "/app/operations", actionLabel: "ოპერაციების გახსნა" },
...
export function GuidedHelpTour({ organizationId, role, open, onOpenChange }: { organizationId: string; role: TourRole; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    if (!state.data.completed && !state.data.autoShowDisabled) onOpenChange(true);
