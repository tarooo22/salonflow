import { CalendarDays, CheckCircle2, TrendingUp, UsersRound } from "lucide-react";
import { useState, type CSSProperties, type PointerEvent } from "react";

type SceneStyle = CSSProperties & { "--scene-x": string; "--scene-y": string };

const appointments = [
  { time: "10:30", name: "ნინო ბერიძე", service: "თმის შეჭრა", tone: "fuchsia", price: "40 ₾" },
  { time: "12:00", name: "ლука ჯავახი", service: "საღებავი", tone: "violet", price: "120 ₾" },
  { time: "14:15", name: "ანა გელაშ.", service: "მანიკიური", tone: "teal", price: "45 ₾" },
];
...
export function SalonFlowHeroScene() {
  ...
  return <div className="sf-hero-scene" style={style} onPointerMove={handleMove} onPointerLeave={() => setDepth({ x: 0, y: 0 })} aria-label="SalonFlow-ის სამუშაო სივრცის ვიზუალური მიმოხილვა">
      ...
      <div className="sf-hero-scene__main">
        ...
        <div className="sf-hero-scene__calendar">
          ...
          <div className="sf-hero-scene__agenda" aria-hidden="true">
            {appointments.map(item => <div key={item.time} className={`sf-hero-scene__slot sf-hero-scene__slot--${item.tone}`}><span className="sf-hero-scene__slot-time">{item.time}</span><span className="sf-hero-scene__slot-body"><strong>{item.name}</strong><small>{item.service}</small></span><span className="sf-hero-scene__slot-price">{item.price}</span></div>)}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="sf-hero-scene__float sf-hero-scene__float--team"><span className="sf-hero-scene__float-icon sf-hero-scene__float-icon--violet"><UsersRound aria-hidden="true" /></span><div><p>გუნდი</p><strong>4 სპეციალისტი</strong></div></div>
    <div className="sf-hero-scene__float sf-hero-scene__float--growth"><span className="sf-hero-scene__float-icon sf-hero-scene__float-icon--pink"><TrendingUp aria-hidden="true" /></span><div><p>ზრდა</p><strong>+21% თვეში</strong></div></div>
