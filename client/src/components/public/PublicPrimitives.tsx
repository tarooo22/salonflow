...
export function SalonFlowMark({ inverted = false }: { inverted?: boolean }) {
  return <Link href="/" className="inline-flex items-center gap-2 rounded-lg font-semibold tracking-tight">
    <span className={`grid size-9 place-items-center rounded-xl border ${inverted ? "border-white/10 bg-white/8 text-[var(--sf-terracotta-strong)]" : "border-[var(--sf-line)] bg-[var(--sf-surface-raised)] text-[var(--sf-terracotta-strong)]"}`}><CalendarDays className="size-4" aria-hidden="true" /></span>
