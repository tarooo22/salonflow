...
export function SalonFlowMark({ inverted = false, compact = false }: { inverted?: boolean; compact?: boolean }) {
  return <Link href="/" className={`sf-public-brand inline-flex shrink-0 items-center gap-2 rounded-xl font-semibold tracking-tight focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sf-focus)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--sf-bg)] ${compact ? "sf-public-brand--compact" : ""}`}><span className={`sf-brand-mark ${inverted ? "sf-brand-mark--inverted" : ""}`} aria-hidden="true"><i /><i /><i /></span><span className="text-lg">SalonFlow</span></Link>;
}
...
export function PublicHeader() {
  ...
  const navigation = [{ href: "/salons", label: t("salons") }, { href: "/partner", label: "პარტნიორი" }, { href: "/features", label: t("features") }, { href: "/pricing", label: t("pricing") }, { href: "/demo", label: t("preview") }, { href: "/faq", label: t("faq") }, { href: "/contact", label: t("contact") }];
  useEffect(() => {
  ...
  return <><a className="sf-skip-link" href="#main-content" onClick={skipToContent}>{t("skip")}</a><header className={`sf-public-header sticky top-0 z-30 border-b backdrop-blur-xl ${isScrolled ? "sf-public-header--scrolled" : ""}`}><div className="sf-public-container flex min-h-18 items-center justify-between gap-3 py-3 lg:min-h-20"><SalonFlowMark compact={isScrolled} /><div className="hidden min-w-0 flex-1 items-center justify-between gap-3 min-[1600px]:flex"><nav className="min-w-0 items-center gap-0.5 min-[1600px]:flex" aria-label={t("mainNav")}>{navigation.map(item => <Link key={item.href} className={`sf-public-nav-link sf-interactive rounded-xl px-3 py-2.5 text-sm font-semibold ${location === item.href ? "sf-public-nav-link--active" : ""}`} href={item.href} aria-current={location === item.href ? "page" : undefined}>{item.label}</Link>)}</nav><div className="flex shrink-0 items-center gap-2"><Link href={bookingHref} className="sf-public-booking-link sf-interactive inline-flex min-h-10 items-center whitespace-nowrap px-3.5 py-2 text-sm font-semibold">{t("onlineBooking")}</Link><span className="h-7 w-px bg-[var(--sf-line)]" aria-hidden="true" /><PublicLanguageSelector /><Button asChild variant="publicQuiet" className="h-10 px-3"><Link href="/login">{t("signIn")}</Link></Button><Button asChild variant="public" className="h-10 px-4"><Link href="/register">{t("startFree")}</Link></Button></div></div><div className="hidden shrink-0 items-center gap-2 md:flex min-[1600px]:!hidden"><Link href={bookingHref} className="sf-public-booking-link sf-public-compact-booking sf-interactive inline-flex min-h-10 items-center whitespace-nowrap px-3.5 py-2 text-sm font-semibold">{t("onlineBooking")}</Link><PublicLanguageSelector className="hidden lg:inline-flex" /></div><Button type="button" variant="publicQuiet" size="icon" className="min-[1600px]:!hidden" aria-expanded={menuOpen} aria-controls="public-mobile-menu" aria-label={menuOpen ? t("closeMenu") : t("openMenu")} onClick={() => setMenuOpen(open => !open)}>{menuOpen ? <X aria-hidden="true" /> : <Menu aria-hidden="true" />}</Button></div>{menuOpen ? <div id="public-mobile-menu" className="sf-motion-enter border-t border-[var(--sf-salon-hairline)] bg-[var(--sf-surface-raised)] min-[1600px]:!hidden"><nav className="sf-public-container grid gap-1 py-3" aria-label={t("mobileNav")}><Link href={bookingHref} onClick={() => setMenuOpen(false)} className="sf-public-booking-link sf-public-compact-booking flex min-h-12 items-center px-4 py-3 text-sm font-semibold">{t("onlineBooking")}</Link><Link href="/salons" onClick={() => setMenuOpen(false)} className="sf-public-discovery-shortcut sf-interactive flex min-h-12 items-center px-4 py-3 text-sm font-semibold">{t("salons")}</Link>{navigation.filter(item => item.href !== "/salons").map(item => <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className={`sf-public-nav-link rounded-xl px-3 py-3 text-sm font-semibold ${location === item.href ? "sf-public-nav-link--active" : ""}`} aria-current={location === item.href ? "page" : undefined}>{item.label}</Link>)}<div className="mt-2 flex items-center justify-between gap-2"><PublicLanguageSelector /><PwaInstallButton /></div><div className="mt-2 grid grid-cols-2 gap-2"><Button asChild variant="publicSecondary"><Link href="/login">{t("signIn")}</Link></Button><Button asChild variant="public"><Link href="/register">{t("start")}</Link></Button></div></nav></div> : null}</header></>;
}

export function PublicFooter() {
  const { t } = usePublicLocale();
  return <footer className="border-t border-[var(--sf-line)] bg-[var(--sf-bg)] text-[var(--sf-ink)]"><div className="sf-public-container grid gap-8 py-10 md:grid-cols-[1.4fr_repeat(2,minmax(0,1fr))]"><div><SalonFlowMark inverted /><p className="mt-4 max-w-sm text-sm leading-6 text-[var(--sf-muted)]">{t("footer")}</p></div><div><h2 className="text-sm font-semibold">{t("product")}</h2><div className="mt-3 grid gap-2 text-sm text-[var(--sf-muted)]"><Link href="/salons">{t("salons")}</Link><Link href="/partner">პარტნიორი სალონებისთვის</Link><Link href="/features">{t("features")}</Link><Link href="/demo">{t("preview")}</Link><Link href="/book">{t("onlineBooking")}</Link><Link href="/register">{t("workspaceCreate")}</Link></div></div><div><h2 className="text-sm font-semibold">{t("information")}</h2><div className="mt-3 grid gap-2 text-sm text-[var(--sf-muted)]"><Link href="/pricing">{t("pricing")}</Link><Link href="/faq">{t("faq")}</Link><Link href="/contact">{t("contact")}</Link><Link href="/privacy">Analytics კონფიდენციალურობა</Link><Link href="/login">{t("workspaceLogin")}</Link></div></div></div><div className="border-t border-[var(--sf-line)]"><div className="sf-public-container flex flex-wrap items-center justify-between gap-3 py-4 text-xs text-[var(--sf-muted)]"><span>{t("footer")}</span><span>© {new Date().getFullYear()} SalonFlow</span></div></div></footer>;
}
